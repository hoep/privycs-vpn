...
```bash
# dnf resolves the GUI libraries and pulls the recommended VPN tools:
sudo dnf install ./privycs-vpn-linux-amd64-<version>.rpm

# openSUSE:
sudo zypper install --allow-unsigned-rpm ./privycs-vpn-linux-amd64-<version>.rpm
```

The `.rpm` installs to the same `/usr/local/bin/privycs-vpn` and requires the
GTK / WebKit libraries by their shared-object names (so it resolves on both
Fedora and openSUSE without hard-coding either one's package names). The package
is unsigned, hence `--allow-unsigned-rpm` on zypper; dnf installs a local file
...
- **v1.1.5.116** *(Linux)* — **Now shipping an `.rpm` package for Fedora, RHEL and openSUSE.** Alongside the existing `.deb`, every release now carries a `privycs-vpn-linux-amd64-<version>.rpm`. The one-line installer (`install-linux.sh`) uses it automatically on dnf and zypper systems, so those distributions get proper dependency resolution and clean uninstall instead of a bare binary drop. The package declares its GTK/WebKit requirements by shared-object name, which resolves on both Fedora and openSUSE without guessing at each one's package names. No change on Debian/Ubuntu or on the app itself.
- **v1.1.5.115** *(all desktop OSes)* — **The in-app Help page now sanitises what it displays, and a profile name can no longer smuggle commands into the Windows VPN setup.** **(1)** The Help screen loads its documentation from privycs.com and rendered it directly. If that document were ever tampered with — a break-in on the website, or an intercepted connection — the altered content could have run code inside the app. The page now strips anything executable out of the document before showing it, while leaving the normal formatting, tables and links untouched. **(2)** On Windows, setting up an IPSec connection passed the connection's name and server address to the system in a way that a deliberately crafted name (containing a quote character) could have escaped, turning part of the name into a command. Both values are now escaped, as they already were everywhere else in the same file. Names like that do not occur in normal use; this closes the door on a config file from an untrusted source.
- **v1.1.5.114** *(all desktop OSes)* — **Security maintenance: updated the libraries that render the in-app Help page, and hardened the build pipeline.** The Help screen pulls its documentation from privycs.com and renders it with a Markdown library; that library and its link parser had published fixes for a denial-of-service and a parsing flaw, so both are now updated. The charting and build-tooling libraries were brought up to date in the same pass — the app now reports no known vulnerable dependencies at all. Separately, the automated build now refers to each of its build steps by an exact, immutable identifier rather than a moving label, so a change made upstream to one of those steps can no longer silently alter how Privycs is built and signed. No functional change to the app.
- **v1.1.5.110** *(all desktop OSes)* — **Search box for the gateway config list.** Pulling configs from the gateway used to mean scrolling an unfiltered list — painful with a Pro account that carries a hundred or more. There is now a search field above the list, wherever gateway configs are shown: when creating a connection, when adding another protocol to an existing one, and in the gateway panel on the Connections screen. It matches on both the config name and the interface name, since peer names are often near-identical ("laptop", "laptop-2") and the interface name is what actually tells them apart.
- **v1.1.5.109** *(all desktop OSes)* — **Linux: IPSec connects again — and the live speed readout stops flickering.** **(1)** On Linux, connecting an IPSec profile that excludes some networks from the tunnel always failed with "peer not responding", while the identical profile connects on Windows, macOS, iOS and Android. The exclusions were negotiated with the server as part of the key exchange, and with a few hundred of them the negotiation message grew to about 11 KB — far too large to be answered, so the server stayed silent and every attempt timed out. The excluded networks are now applied locally (as Windows and macOS have always done) and the key exchange carries a single entry again. **(2)** **Live speed readout no longer flickers between 0 B/s and a spike.** The Traffic figure jumped between 0 B/s and an implausibly high value about once a second even while a steady download was running. Nothing was wrong with the tunnel — only the way the speed was derived. Three separate causes, all fixed: a refresh triggered by the interface (opening a screen, switching protocol) re-read the byte counters milliseconds after the regular one-second sample, saw no new bytes and reported 0 B/s; a brief dip in the counters (normal during a key renegotiation) was taken as the new starting point, so the next honest reading was billed all the catch-up bytes at once and showed as a spike; and a genuine counter reset could leave the readout stuck at 0 until the next disconnect. The speed is now sampled on a single cadence, ignores transient counter dips while still recovering from real resets, and reads steady.
- **v1.1.5.108** *(Linux fix; all desktop OSes rebuilt)* — **Split-tunnel WireGuard/AmneziaWG connects in a second on Linux instead of half a minute.** Connecting a config that excludes some networks from the tunnel took 25–30 seconds on Linux, and disconnecting took just as long — while the very same config connects instantly on Windows and macOS. The tunnel itself was never slow: a split-tunnel config lists the networks to route as one very long line (several hundred entries, ~10 KB), and on Linux that line is parsed by a shell script whose whitespace handling gets dramatically slower the longer the line is — 22 seconds burnt on text processing before a single packet moved. The network list is now written as many short lines instead of one long one, which the tunnel treats identically. Windows and macOS never ran that script and were always fast. (Full-tunnel configs were never affected — their network list is short.)
- **v1.1.5.107** *(Linux fix; all desktop OSes rebuilt)* — **AmneziaWG on Linux connects properly again (no more 60-second hang).** With the AmneziaWG userland installed but no kernel module, the tunnel came up and passed traffic perfectly — but the app never noticed: it waited a full minute, declared the connection failed and started failing over to other protocols, and the next attempt then hit "interface already exists" (because the tunnel really was up). The cause: the app waited on the output stream of the tool that brings the tunnel up, and the AmneziaWG background process keeps that stream open for as long as the tunnel lives, so the wait never ended. The tunnel status is now read correctly, so AmneziaWG connects and is reported as connected. A leftover interface from an earlier stuck attempt is also cleaned up and retried automatically. (Plain WireGuard was never affected — it uses the kernel module and leaves no background process.)
- **v1.1.5.106** *(Linux/macOS fix; all desktop OSes rebuilt)* — **No more bogus "wg-quick down failed" errors on disconnect.** Tearing a WireGuard/AmneziaWG tunnel down reported an error whenever the tunnel wasn't actually up — which is exactly what happens after a connection attempt that failed before the network interface was created (e.g. AmneziaWG without its userland installed), or when disconnecting something already disconnected. Nothing was wrong: there was simply nothing left to tear down. Teardown is now idempotent, so "already down" counts as success and only genuine failures are reported.
- **v1.1.5.105** *(Linux fixes; all desktop OSes rebuilt)* — **Linux: black app window fixed, and Quit really quits.** **(1)** On some Linux machines the app opened as an all-black window — it was running fine, but the embedded browser (WebKitGTK 2.42+) failed to paint through its newer GPU rendering path on drivers with limited OpenGL support (open-source `nouveau`, software rendering, VMs, remote sessions). The app now disables that rendering path on Linux, so the interface shows up. (Set `WEBKIT_DISABLE_DMABUF_RENDERER=0` to opt back in.) **(2)** Choosing **Quit** in the tray shut everything down (tunnel down, kill switch off) but left the process running in the background, so the app never really exited. It now terminates for real. macOS/Windows unaffected.
- **v1.1.5.104** *(Linux fix; all desktop OSes rebuilt)* — **Linux: importing an IPSec config no longer breaks WireGuard/OpenVPN.** On Linux, loading a gateway IPSec profile installed kernel IPsec "trap" policies covering almost the entire internet (the full-tunnel route set), which then black-holed traffic to the WireGuard/OpenVPN servers — so after an IPSec config was imported, connecting via another protocol (or automatic failover) silently carried no traffic. IPSec now installs policies only for an actually-negotiated tunnel (on connect), not merely on import, so a loaded-but-not-connected IPSec profile can't interfere with the other protocols. (macOS/Windows were unaffected — they don't use this mechanism.) Also fixed a WireGuard endpoint-bypass route error on multi-homed machines (several default routes). macOS/Windows behaviour unchanged.
- **v1.1.5.103** *(Linux protocol fixes; all desktop OSes rebuilt)* — **Linux: gateway IPSec configs load again, OpenVPN gets its IPv4 route.** **(1)** IPSec configs pulled from a Privycs Gateway failed to import on Linux — the client certificate was never extracted from the config's PKCS#12 bundle, so strongSwan couldn't load the connection. It's now extracted the same way macOS and Windows already do, and the config imports and connects. **(2)** OpenVPN tunnels came up but installed no IPv4 default route: the privileged helper launched OpenVPN with a minimal `PATH`, so OpenVPN couldn't find the `ip`/`route` tools it needs to apply the pushed route. The helper now gives OpenVPN the same `PATH` that WireGuard already had, so the default route lands. **(3)** WireGuard/AmneziaWG configs whose server endpoint is a bare IPv6 address now get their endpoint-bypass route (previously only added when a v4 endpoint was present). macOS and Windows behaviour is unchanged.
...
- **v1.1.5.97** *(cross-platform fix; all desktop OSes rebuilt)* — **Renaming a connection no longer crashes the Configs screen.** Double-clicking a connection's name to rename it could blank out the whole Configs list. The rename field's auto-focus call was made against the wrong target (a collection of elements rather than the single field), which threw and tore the screen down. Auto-focus now targets the actual input, so rename works without disturbing the list.
- **v1.1.5.96** *(cross-platform fix; all desktop OSes rebuilt)* — **Pool import follow-up — the "Add Pool" screen vanished the moment you picked a ZIP.** Completes the v1.1.5.95 fix: with the native file dialog now actually delivering the chosen file, the selected-files line rendered for the first time and hit a long-latent translation bug (a legacy `$tc` plural call that does not exist under this app's i18n mode), which tore the whole "Add Pool" screen down before you could press Import. Switched to the supported translation call; picking a ZIP (or several configs) now keeps the screen and imports as expected.
- **v1.1.5.95** *(cross-platform fix; all desktop OSes rebuilt)* — **Connection-pool import (ZIP or multiple configs) works again — silent no-op fixed.** Importing a pool from a `.zip` archive (or selecting several config files at once) could do nothing at all: no pool created, no error message, nothing in the log. The cause was that the embedded webview never actually handed the chosen file to the app — on the click-to-browse path the in-browser file read could fail silently on some platforms (notably macOS), and dragging files onto the drop-zone delivered an empty list because the OS file-drag was intercepted natively. Both paths now go through the operating system directly: clicking the drop-zone opens the native OS file dialog, and dropped files arrive as real file paths — so the app reads them on its own side with no in-browser copy, which also handles large provider ZIPs reliably.
- **v1.1.5.93** *(cross-platform reliability; all desktop OSes rebuilt)* — **Kill switch no longer locks you out after an app update.** The kill-switch traffic block runs in the privileged helper and survives an app update; if the freshly-updated build couldn't immediately reconnect, the block could keep all traffic off until you toggled the kill switch off by hand. The client now detects an update on startup and *defers* the startup block until a clean reconnect re-arms it — so an update can't strand you offline. Steady-state kill-switch protection (blocking on an unexpected tunnel drop) is unchanged.
- **v1.1.5.89** *(cross-platform security + reliability; all desktop OSes rebuilt)* — **Kill switch now fails CLOSED, crash reports actually strip Wi-Fi names, clearer Linux setup.** **(1) Kill switch fail-closed** — on an *unexpected* tunnel drop (detected by the connectivity probe), the client now blocks all traffic *before* it tears the dead tunnel down and tries to reconnect; previously there was a window where traffic could briefly exit on your physical connection in the clear (and indefinitely if the reconnect failed). A successful reconnect lifts the block automatically; a failed one keeps it engaged. No change when the kill switch is off. **(2) Crash-report SSID redaction** — the opt-in diagnostics pipeline now genuinely strips Wi-Fi network names (SSIDs) before upload, matching the privacy policy (the desktop reporters previously claimed this but didn't do it). **(3) Linux setup hint** — if a protocol's system package is missing (WireGuard/OpenVPN/strongSwan tools), the connect error now tells you exactly what to install instead of a generic "not available"; OpenVPN transfer stats also read the correct tunnel interface when more than one exists.
- **v1.1.5.88** *(macOS-only IPSec fixes; macOS rebuild only)* — **macOS IPSec: DNS restored on disconnect + the repeated authorization prompt fixed properly.** **(1) DNS restored on disconnect** — macOS left the gateway-pushed DNS server active after the IPSec tunnel went down (the resolver entry wasn't torn down), so name resolution stayed broken on a now-dead DNS server until reboot. The client now snapshots your DNS *before* connecting and restores it *after* disconnecting (reverting to your LAN/DHCP resolver). **(2) Per-connect authorization prompt** — the v1.1.5.87 attempt to skip the redundant configuration-save (the cause of the repeated macOS keychain/authorization prompt) compared a field that doesn't survive a configuration reload, so it never actually skipped; the comparison now uses fields that do, so reconnecting to the same server no longer re-saves the configuration or re-imports the certificate, and therefore no longer prompts. macOS-only; Linux + Windows version files unchanged. *(The same DNS-restore behaviour for Windows is tracked separately — Windows IPSec uses a different DNS mechanism.)*
- **v1.1.5.87** *(macOS-only IPSec fixes; macOS rebuild only)* — **macOS IPSec: live traffic counter, fewer connect prompts, reachable excluded networks.** Three fixes for the macOS IKEv2/IPSec path (Apple NEVPNManager). **(1) Traffic counter updates again** — the byte counter read the wrong network interface for IPSec (the Apple stack names it `ipsecN`, not `utunN`), so it sat at 0 B/s even while data flowed; it now resolves the correct interface. **(2) No repeated authorization prompt on reconnect** — the connection was re-saved (and its certificate re-imported into the keychain) on every connect, which made macOS ask for authorization each time; the save is now skipped when the configuration is unchanged, so reconnecting to the same server no longer prompts. Switching to a *different* IPSec server still saves once — that is a macOS requirement for directly-distributed (non-App-Store) apps. **(3) Excluded networks reachable again** — after the v1.1.5.86 full-tunnel fix the default route pulled the split-tunnel bypass subnets into the tunnel; per-subnet bypass routes via the physical gateway are now installed on connect (and removed on disconnect), matching the Windows and Android behaviour. macOS-only; Linux + Windows version files unchanged.
- **v1.1.5.86** *(macOS-only IPSec fix; macOS rebuild only)* — **macOS IPSec now actually passes traffic (full-tunnel) + IPv6 kill-switch corrected.** The macOS IKEv2 tunnel reported "connected" but carried no traffic and failed over to WireGuard after ~20 s — the Apple stack had been left to route only the gateway's pushed config-mode subnet instead of the whole tunnel. It now routes all traffic through the tunnel, matching the iOS client. Also corrected the always-on IPv6 kill-switch on macOS: a firewall-rule (`pf`) syntax error that stopped the rule from loading, plus a false "IPv4-only" detection that could block IPv6 even when the tunnel itself carries IPv6. macOS-only; Linux + Windows version files unchanged.
- **v1.1.5** *(cross-platform; all desktop OSes rebuilt)* — **Security & stability hardening (production-readiness pass).** The gateway URL is now HTTPS-only (your API key can never be sent over plaintext `http://`, except to localhost); the IPv6 kill-switch is applied *before* the connection is reported up, closing a brief leak window on IPv4-only tunnels; the internal connection registry is now thread-safe; privileged-helper input validation was tightened (path-traversal + request-size limits) and the OpenVPN shutdown verifies the process before signalling it; and the license signer was removed from the shipped binary (verify-only). No user-facing behaviour change.
- **v1.1.4** *(cross-platform; all desktop OSes rebuilt)* — **Coordinated 1.1.4 release — version alignment.** No desktop-side functional changes; rebuilt to share the 1.1.4 version with the iOS release (home-screen-widget protocol icons) and Android. The Connect screen and all other text were already fully localized.
- **v1.1.3.24** *(cross-platform; all desktop OSes rebuilt)* — **Deleting an IPSec connection now removes its server config and certificates too.** Building on v1.1.3.23's config-file cleanup, deleting an IPSec connection now also clears the OS-level IPSec artifacts via the privileged helper: on macOS and Linux the strongSwan configuration and certificate files, and on Windows the all-user VPN entry plus the Privycs-issued certificates. The Windows certificate removal is scoped strictly to Privycs-owned certificates, so publicly-trusted roots are never touched. (Windows split-DNS / NRPT rules added by a gateway setup script are not yet removed automatically.)
