...
- **v1.1.5.111** — **IPSec configs from the gateway now connect straight from the download — no certificate install, no password typing.** Setting up an IPSec connection used to mean four system dialogs: the VPN permission, then Android's certificate-install screen where you had to paste a password the app showed you in a pop-up, then setting a screen lock if you had none, and finally picking your new certificate out of a list of every credential on the device. Now there is one dialog — the VPN permission Android always asks for, once — and everything after it is automatic. Subsequent connects ask nothing at all. This also fixes three things that followed from the old design: re-downloading a config now really applies the new certificate (previously the old one was silently kept, so a certificate renewal on the gateway never reached the phone); deleting a connection no longer strands a certificate in Android's credential store that the app had no way to remove; and IPSec now works for pool members, which previously failed outright because the install dialog had nowhere to appear. **Upgrading:** nothing to re-download — your existing IPSec connections just work. The certificate a previous version installed becomes unused; the app shows a one-time note so you can remove it by hand in Android Settings if you want the tidy-up. It is inert either way.
