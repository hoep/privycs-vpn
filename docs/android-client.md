...
- **v1.1.5.118** — **Fixed a background crash that could kill the VPN when a scheduled pool rotation fired.** On newer Android, a foreground service must announce itself within five seconds of being started, or the system force-stops the app. When a connection pool's automatic rotation timer went off while the app was in the background — with the screen off, on a flaky network — the service started but didn't make that announcement in time on a few code paths, so Android killed it. The service now announces itself immediately on every path, showing the connection it's already running so nothing on screen flickers. This mainly affected pool users with rotation enabled; single connections were rarely hit.
- **v1.1.5.113** — **Internal: fixed a latent database problem that would have broken the app on a future update.** The app bundles strongSwan, and our build had claimed a schema version number that strongSwan itself had not reached yet. The moment a future strongSwan release used that same number for its own changes, existing installations would have skipped them and the app would have crashed on startup with a missing-column error. Nothing was wrong today — this only bites on a later update, which is exactly why it was worth fixing before it could. The version number is now left to strongSwan, and the app checks the database itself for what it needs rather than trusting a counter. Existing databases are adopted and repaired silently on first launch; nothing to do, no data touched.
- **v1.1.5.111** — **IPSec configs from the gateway now connect straight from the download — no certificate install, no password typing.** Setting up an IPSec connection used to mean four system dialogs: the VPN permission, then Android's certificate-install screen where you had to paste a password the app showed you in a pop-up, then setting a screen lock if you had none, and finally picking your new certificate out of a list of every credential on the device. Now there is one dialog — the VPN permission Android always asks for, once — and everything after it is automatic. Subsequent connects ask nothing at all. This also fixes three things that followed from the old design: re-downloading a config now really applies the new certificate (previously the old one was silently kept, so a certificate renewal on the gateway never reached the phone); deleting a connection no longer strands a certificate in Android's credential store that the app had no way to remove; and IPSec now works for pool members, which previously failed outright because the install dialog had nowhere to appear. **Upgrading:** nothing to re-download — your existing IPSec connections just work. The certificate a previous version installed simply goes unused — it is inert, and you can delete it in Android Settings if you like a tidy device.
- **v1.1.5.110** — **Search box for the gateway config list.** Pulling configs from the gateway used to mean scrolling an unfiltered list — painful with a Pro account that carries a hundred or more. There is now a search field above the list, wherever gateway configs are shown: when creating a connection, when adding another protocol to an existing one, and in the gateway panel on the Connections screen. It matches on both the config name and the interface name, since peer names are often near-identical ("laptop", "laptop-2") and the interface name is what actually tells them apart. **Also fixed: the gateway list could shuffle its own rows.** With many configs, scrolling the list swapped entries around, showed the wrong details on a row, and made rows disappear or appear twice. The gateway numbers its configs per protocol, so an IPSec and an OpenVPN config both numbered 2 are normal — but the list treated that number as the row's identity, so those rows were indistinguishable to it. Rows are now identified per protocol.
- **v1.1.5.109** — **Live speed readout no longer flickers between 0 B/s and a spike.** The Traffic figure jumped between 0 B/s and an implausibly high value about once a second even while a steady download was running. Nothing was wrong with the tunnel — only the way the speed was derived. Three separate causes, all fixed: a refresh triggered by the interface (opening a screen, switching protocol) re-read the byte counters milliseconds after the regular one-second sample, saw no new bytes and reported 0 B/s; a brief dip in the counters (normal during a key renegotiation) was taken as the new starting point, so the next honest reading was billed all the catch-up bytes at once and showed as a spike; and a genuine counter reset could leave the readout stuck at 0 until the next disconnect. The speed is now sampled on a single cadence, ignores transient counter dips while still recovering from real resets, and reads steady.
- **v1.1.5.100** — **Coordinated all-platform rebuild — version alignment.** iOS, Apple TV, Mac, Android and Desktop now share one version (1.1.5.100), all rebuilt from scratch. No functional Android changes vs v1.1.5.98.
- **v1.1.5.98** — **Security hardening pass.** Three defensive fixes, no behaviour change: **(1)** updated the bundled cryptography library (BouncyCastle) to a patched release that closes a timing-side-channel advisory in an algorithm the app doesn't even use — done purely to stay off the vulnerable version range. **(2)** Gateway requests (Pro config sync + Apple-TV enrollment) no longer follow HTTP redirects, so your API key can't be bounced to a host you didn't configure. **(3)** Internal widget-refresh broadcasts are now explicitly scoped to the app, so they're never visible to other apps. Shipped alongside Desktop v1.1.5.98.
- **v1.1.5** — **Stability hardening (production-readiness pass).** Thread-safety fixes so the WireGuard/AmneziaWG and IPSec status always read a consistent uptime/traffic value; logging moved off the UI thread (less jank under load) with the per-second status log demoted to debug; faster cold start (settings are pre-warmed so the first connect/disconnect can't briefly stall); and two edge-case fixes — a timeout on a stuck IPSec disconnect, and no more false "IPv6 leak" banner if you switch away within a few seconds. No behaviour change.
- **v1.1.4** — **Coordinated 1.1.4 release.** Localization consistency: the supported-formats protocol labels on the Add-Configuration screen now resolve from the shared string resources like everything else. The headline of this coordinated release is on iOS (home-screen-widget protocol icons); the Android app is otherwise functionally unchanged.
- **v1.1.3.23** — **IPSec connects to Let's-Encrypt gateways + cleans up on delete.** **(1)** Some gateways present a publicly-trusted (Let's Encrypt) IKEv2 server certificate but send only the leaf certificate — Android couldn't obtain the missing intermediate (its credential store strips foreign CAs out of the client bundle, and this build has no online fetch), so the connection failed with a trust error. The gateway now ships the full public certificate chain inside the profile and the client imports it into strongSwan's own trust store, so the chain validates end-to-end and IPSec connects. **(2)** Deleting an IPSec connection now also removes what it left behind — the strongSwan profile, the imported CA chain, and the stored certificate-alias reference — instead of leaving those on the device forever. (The client certificate you installed into Android's own credential store still has to be removed by you in Settings; no app can delete it for you.)
- **v1.1.3.22** — **IPSec: re-prompts for the client certificate if it was removed.** If the VPN client certificate was deleted from Android's system credential store (e.g. via *Settings → Encryption & credentials → Clear credentials*, or removed by hand), the app previously kept trying to use the now-missing certificate and silently failed over instead of connecting — and never asked you to re-install it. The connect path now verifies the remembered certificate still resolves in the KeyChain; if it doesn't, it forgets the stale reference and re-runs the install prompt so you can re-add the certificate in one step.
- **v1.1.3.21** — **IPSec: connect to gateways that use a publicly-trusted server certificate.** Gateways can now present a publicly-trusted (e.g. Let's Encrypt) IKEv2 server certificate instead of a self-signed one. The client reads the server trust anchor that the gateway ships inside each profile and validates the server explicitly against it, so IPSec now connects to these gateways — previously the handshake never started. Self-signed gateways keep working exactly as before, and because the profile identity is now stable across re-downloads you don't have to re-install the client certificate. No action needed beyond re-downloading the profile.
- **v1.1.3** — **Fixes a crash on connect.** A connection attempt could crash the app while the Smart Decision Engine observed the connect transition (an unguarded engine call on the connect path). The engine is a passive observer and can now never bring down the VPN connection — its observation is fully isolated. Shipped alongside iOS v1.1.3 (which adds automatic protocol failover and fixes pool import + pool-only connect on iPhone/iPad).
- **v1.1.2** — **Smart Decision Engine: adaptive scoring + roaming awareness, plus a connect-crash fix.** With Automatic protocol selection on, the engine now learns per-network: a protocol that repeatedly fails to establish on the current network is demoted on the next attempt (an in-memory per-network success/fail score, reset when the network changes), and on mobile data IPSec is promoted so the tunnel survives Wi-Fi↔cellular switches via MOBIKE. This release also fixes a crash that could occur right as a connection was being established (an internal threading race in the new engine). Manual protocol selection (engine off) is unchanged. Shipped alongside Desktop and iOS v1.1.2.
- **v1.0.8.4** — **Android TV / Google TV support (in testing).** The app now ships an Android TV / Google TV form factor: a D-pad/remote-navigable living-room UI (leanback launcher) reusing the same VPN engine as the phone app. It pulls configs from a Privycs gateway with a TV-friendly enroll flow (**gateway URL + access token**; QR scan, per-app VPN and the network-rules screen are dropped on TV). Same APK/AAB as the phone app — no separate install. *In testing:* not yet enabled for the Play Store's Android TV section, and not yet hardware-verified.
- **v1.0.8.2** — **Localized country names + the last service-layer strings translated.** The server country label shown next to the flag now uses Android's own localized country names (CLDR via `Locale.getDisplayCountry`) instead of a fixed English list, so it follows your in-app language. A localization sweep also moved 21 remaining hard-coded English strings — VPN notifications, Kill-Switch states and the Quick Settings Tile labels — into all six languages (en/de/es/fr/it/pt). No behavioural change. Shipped in lockstep with Desktop v1.0.8.2 and the iOS v1.0.8.3 home-screen-widgets release.
- **v1.0.8** — **In-app Help links fixed; version aligned to 1.0.8.** The Help screen renders the live guide, which cross-links to sibling docs with relative `.md` paths — tapping those did nothing before; they now open the correct page in your browser. Version bumped to 1.0.8 alongside the coordinated iOS / Desktop release (no other Android-side functional changes).
