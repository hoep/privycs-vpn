...
On iOS/macOS the kill switch is a real OS-level block: when enabled, the NetworkExtension is configured with *include-all-networks*, so the system routes **all** traffic through the tunnel and **drops** any traffic whenever the tunnel is down or still (re)connecting — nothing leaks onto the open network. Your local network (LAN / printers / AirPlay) stays reachable. A manual disconnect turns the kill switch off for that session (so iOS doesn't immediately reconnect the tunnel you just tore down).
...
- **v1.1.5.110** — **Search box for the gateway config list.** Pulling configs from the gateway used to mean scrolling an unfiltered list — painful with a Pro account that carries a hundred or more. There is now a search field above the list, wherever gateway configs are shown: when creating a connection, when adding another protocol to an existing one, and on the Apple TV config screen. It matches on both the config name and the interface name, since peer names are often near-identical ("laptop", "laptop-2") and the interface name is what actually tells them apart. **Also fixed: the gateway list could shuffle its own rows.** With many configs, scrolling the list swapped entries around, showed the wrong details on a row, and made rows disappear or appear twice. The gateway numbers its configs per protocol, so an IPSec and an OpenVPN config both numbered 2 are normal — but the list treated that number as the row's identity, so those rows were indistinguishable to it. Rows are now identified per protocol. The same mix-up put the "imported" checkmark on every config sharing a peer name (the same peer usually exists as WireGuard, OpenVPN and IPSec), and on Apple TV it could resolve a selection to the wrong config entirely — pick OpenVPN, connect IPSec.
- **v1.1.5.109** — **Live speed readout no longer flickers between 0 B/s and a spike.** The Traffic figure jumped between 0 B/s and an implausibly high value about once a second even while a steady download was running. Nothing was wrong with the tunnel — only the way the speed was derived. Three separate causes, all fixed: a refresh triggered by the interface (opening a screen, switching protocol) re-read the byte counters milliseconds after the regular one-second sample, saw no new bytes and reported 0 B/s; a brief dip in the counters (normal during a key renegotiation) was taken as the new starting point, so the next honest reading was billed all the catch-up bytes at once and showed as a spike; and a genuine counter reset could leave the readout stuck at 0 until the next disconnect. The speed is now sampled on a single cadence, ignores transient counter dips while still recovering from real resets, and reads steady. On iPhone, iPad, Apple TV and Mac there was a fourth cause: the tunnel process publishes its counters on its own one-second timer while the app read them on another, so the app regularly re-read the same numbers (0 B/s) and then billed two of the tunnel's intervals to one of its own (double value). The app now times the speed by when the tunnel actually sampled the counters, so the two timers can no longer beat against each other.
- **v1.1.5.101** — **Connect screen shows the protocol pill even for single-protocol connections (iPhone / iPad / Apple TV / Mac).** Previously the protocol badge row only appeared when a connection bundled two or more protocols, so a setup with only WireGuard connections showed no protocol pill at all — unlike Android and Desktop, which always show it. iOS now matches: the badge row appears for any standard connection, showing the active protocol (and still opening the picker when a protocol has multiple endpoints/configs).
- **v1.1.5.100** — **Coordinated all-platform rebuild — version alignment.** iOS, Apple TV, Mac (App Store), Android and Desktop now share one version (1.1.5.100), all rebuilt from scratch. No new iOS features vs the previous beta — this re-ships the latest fixes (real kill switch, cross-platform backup, crash reporting) on the aligned version.
- **v1.1.5.x (latest beta)** — **Real kill switch, reliable cross-platform backup, crash reporting back on.** The **Kill Switch** is now a true OS-level block: with it on, all traffic is forced through the tunnel and iOS/macOS **drop** any traffic when the tunnel is down or still reconnecting — nothing leaks onto the open network (your local network stays reachable). Encrypted **backups now round-trip losslessly** with the Android and Desktop apps — the "last connected" time and active protocol are written in the shared format, so an iOS backup restores on Android (and vice-versa) without errors or lost fields. Anonymous, **opt-in crash reporting** (self-hosted, PII-redacted, default off) is re-enabled after the Xcode 26 toolchain migration. The **Apple TV and Mac (App Store)** builds were rebuilt to pick up the backup fix too. **Kill-switch update fix:** installing a new build with the kill switch on no longer strands connectivity — previously the new VPN engine was blocked out while the system kept blocking all traffic and the auto-reconnect raced the app (so the reconnect wouldn't take). The app now detects the update on launch, clears the stale block + reconnect race first, and a clean reconnect re-arms the kill switch.
- **v1.1.5** — **Security & stability hardening (production-readiness pass).** The home-screen widget no longer keeps VPN configuration (which can include WireGuard/OpenVPN private keys) in unencrypted shared storage — it now reads the config from the Keychain at the moment you switch protocols. Plus: a guard so a quick double-tap can't start two overlapping connects, an OpenVPN connect-timeout so a stalled negotiation can't wedge the tunnel, and small localization/cleanup fixes. No user-facing behaviour change.
- **v1.1.4** — **Home-screen widget shows the real protocol logos.** The widget's connect button and the protocol-switch pills previously used generic system symbols; they now render the actual WireGuard / AmneziaWG / OpenVPN / IPSec brand logos (template-tinted: white when connected, grey when idle), matching the in-app Connect button and the Android widget. Two remaining pool-import hints were also localized into all six languages. Shipped alongside the coordinated Android and Desktop v1.1.4. (Earlier in the 1.1.3.x beta line: IPSec connect + traffic-counter fixes, single-active-VPN enforcement, and connection-delete cleanup across the apps.)
