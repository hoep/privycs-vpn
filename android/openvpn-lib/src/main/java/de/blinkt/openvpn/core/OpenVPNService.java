...
public class OpenVPNService extends VpnService implements StateListener, Callback, ByteCountListener, IOpenVPNServiceInternal {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        // Privycs-VPN patch (v0.9.15.49): exit cleanly on Android-driven
        // sticky restart when there is no profile to resume.
        //
        // Background: when intent == null Android is auto-restarting us
        // because we returned START_STICKY from a previous onStartCommand.
        // ICS-OpenVPN's default behavior in that case is to log
        // "Assuming always on" and stay alive, holding the system VPN
        // slot, even if no profile is available. For the Privycs-VPN
        // multi-protocol pill-switch flow this is catastrophic — when
        // the user switches OpenVPN -> WireGuard, the OpenVPN teardown
        // completes cleanly, then Android auto-restarts this service
        // with null intent (50-100 ms later). The re-started service
        // claims the VPN slot in "always on" limbo, and when the AWG
        // GoBackend$VpnService starts moments later Android destroys
        // the now-redundant *other* VpnServices in the app — which
        // includes PrivycsVpnService. PrivycsVpnService's scope is
        // cancelled, every in-flight coroutine throws
        // JobCancellationException, and the user sees "Job was
        // cancelled" for the new WG/AWG bringup.
        //
        // Fix: if Android auto-restarts us with null intent AND no
        // last-connected profile exists AND no always-on VPN profile
        // is configured, exit synchronously instead of staying alive.
        // The Privycs-VPN client calls
        // ProfileManager.setConnectedVpnProfileDisconnected before
        // tearing down OpenVPN, so the "no last-connected profile"
        // branch is the normal post-disconnect state.
        //
        // Symptom signature this fix targets:
        //   WiFi only, OpenVPN -> WireGuard/AmneziaWG pill switch,
        //   user sees "Connection Failed Job Cancelled".
        if (intent == null) {
            if (ProfileManager.getLastConnectedProfile(this) == null &&
                ProfileManager.getAlwaysOnVPN(this) == null) {
                Log.d("OpenVPN",
                    "Privycs-VPN patch: null intent + no profile + " +
                    "no always-on → stopSelf, returning START_NOT_STICKY");
                stopSelf();
                return START_NOT_STICKY;
            }
        }
