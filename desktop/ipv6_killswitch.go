...
// tunHasIPv6 returns true if the interface owning the given v4
// address ALSO carries a non-link-local IPv6 address (i.e. the
// tunnel itself is dual-stack). Returns false when tunV4 is empty
// (we couldn't identify the tun → assume v4-only and let the
// killswitch err on the safe side).
func tunHasIPv6(tunV4 string) bool {
	if tunV4 == "" {
		return false
	}
